...
| expression-level or pattern-matching conditional forms for `TS007` | `if` expressions, `match`, and comprehension filter clauses blur the boundary between branch smell and ordinary data shaping, so v1 keeps `TS007` on explicit statement-level `if` / `elif` branches only. |
