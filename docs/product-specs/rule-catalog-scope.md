...
| `Magic Number Test` | `TS006` | A standardized test target uses a bare numeric literal directly in a top-level assertion / expectation comparison. v1 inspects bare `assert` comparison operands, `unittest` comparison assertions including `assertIs` / `assertIsNot`, plus top-level `assertTrue` / `assertFalse` comparison conditions, reports the numeric literal location, excludes `0`, `1`, and `-1`, and does not inspect helper-driven nested comparisons, comprehensions, general test data literals, or ordinary call arguments. | `warning` | `high` | The rule remains static and explainable when limited to direct expectation literals with explicit exclusions. |
| `Conditional Logic In Tests` | `TS007` | A standardized test target contains a statement-level `if` / `elif` branch in its executable body after excluding nested definitions. Version 1 reports the finding at the test definition line and excludes `if` expressions, `match` statements themselves, and comprehension filter clauses from the default rule scope; however, a nested statement-level `if` / `elif` that appears inside a `match` case body still counts because the emitted signal is the nested `if` / `elif`. | `warning` | `high` | The signal is explicit from AST statement structure, the reported location is stable, and the remediation is concrete. |
...
| broad magic-number detection outside direct assertion / expectation contexts | Literal values in tests are often intentional test data, so v1 limits `TS006` to direct expectation literals instead of broad numeric-literal scanning. |
| expression-level or pattern-matching conditional forms for `TS007` | `if` expressions, `match` statements themselves, and comprehension filter clauses blur the boundary between branch smell and ordinary data shaping, so v1 keeps `TS007` on explicit statement-level `if` / `elif` branches only. Nested `if` / `elif` branches that appear inside a `match` case body remain in scope because the signal is still the explicit nested branch statement. |
