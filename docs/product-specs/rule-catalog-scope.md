...
| `Magic Number Test` | `TS006` | A standardized test target uses a bare numeric literal directly in an assertion / expectation context. v1 inspects bare `assert` comparison operands plus `unittest` comparison assertions and `assertTrue` / `assertFalse` conditions, reports the numeric literal location, excludes `0`, `1`, and `-1`, and does not inspect general test data literals or ordinary call arguments. | `warning` | `high` | The rule remains static and explainable when limited to direct expectation literals with explicit exclusions. |
