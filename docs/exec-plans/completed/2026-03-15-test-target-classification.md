...
- smell rule の追加実装
- 動的解析や実行時 import 解決
- pytest discovery と明確に矛盾する既存検出挙動の縮小
- 全 rule catalog の確定
...
- `pytest` class-based tests のうち、既存実装が実際に検出していた `Test*` class の `test_*` method を共通 target に残すか
  - 既存挙動との互換性を優先して含める
- 共通判定ロジックの配置先
...
- 2026-03-15: `unittest.TestCase` 判定は `import unittest` / `from unittest import TestCase` に加えて、`unittest.case` module alias のような静的に明白な import も対象に含めた
- 2026-03-15: `pytest` target には top-level `test_*` function / async function に加え、top-level `Test*` class の `test_*` method を含めて既存挙動との互換性を維持した
