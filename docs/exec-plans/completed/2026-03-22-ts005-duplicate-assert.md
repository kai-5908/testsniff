...
ただし現行の採用済み product spec [docs/product-specs/rule-catalog-scope.md](/home/aoi_takanashi/testsniff/docs/product-specs/rule-catalog-scope.md) には `Duplicate Assert` がまだ含まれていない。そのため、この task は rule 実装に加えて、catalog 追加と high confidence を維持できる重複 assertion の仕様境界を同じ change で固める必要がある。
...
- [docs/product-specs/rule-catalog-scope.md](/home/aoi_takanashi/testsniff/docs/product-specs/rule-catalog-scope.md) に `TS005` が反映され、実装とのズレがない
