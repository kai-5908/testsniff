...
- `TS002` は comment を含む placeholder intent を source token から補助的に検出する方向を優先する
- Python 文法上の制約により、純粋な `comment-only function` ではなく、`docstring + comment` のような「非実行テキストしかない test」を主な検出候補として扱う

## 解決した論点

- docstring-only test は `TS001` に残し、`TS002` は comment を伴う placeholder test として定義する
- `comments-only test` を v1.0.0 対象へ昇格させるため、`docs/product-specs/rule-catalog-scope.md` を同一変更で更新する
- `TS001` と `TS002` が同じ location で重なる場合、デフォルトの rule set では `TS002` を優先表示しつつ、`--select TS001` や `--ignore TS002` では `TS001` の既存契約を維持する

## 推奨方針

承認後の実装では、まず Issue #4 を次の実装可能な解釈へ正規化して進める。

- `TS001` の rule semantics と direct selection contract は変更しない
- `TS002` は comment を伴う placeholder test を対象とする
- docstring-only は引き続き `TS001`
- `rule-catalog-scope.md` の対象外記述と rule overlap の扱いは、この方針に合わせて同一変更で更新する
...
- rule 単体テストで `TS001` の既存契約と `TS002` の追加検出を両立できていることを確認する
...
- 2026-03-20: `TS001` の direct selection/ignore contract は維持し、default rule set の重複 location だけ `TS002` を優先表示する
